  ...
  return (
    <div className="bg-flashwhite absolute min-h-screen max-w-[230px] border-r border-gray-300 p-4">
      ...
      <div className="p-3 font-normal flex">
        ...
        />
        <div className="ms-3">
          <p className="font-semibold text-sm text-slate-600">{user.name}</p>
        ...
        <ul className="inline-flex gap-2 flex-col">
          {menuList.map((item, index) => (
            <li
              key={index}
              className={
                "gap-3 cursor-pointer pr-20 items-center px-5 py-2 text-sm text-slate-500 truncate rounded-3xl hover:bg-white inline-flex " +
